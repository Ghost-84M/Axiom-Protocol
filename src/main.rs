...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    // --- Port Binding Logic ---
    if std::env::var("AXIOM_GENESIS_NODE").unwrap_or_default() == "1" {
        // Genesis node: restrict to ports 7000-7003
        let mut current_port: u16 = 7000;
        let max_port: u16 = 7003;
        loop {
            let addr: Multiaddr = format!("/ip4/0.0.0.0/tcp/{}", current_port).parse()?;
            match swarm.listen_on(addr.clone()) {
                Ok(_) => {
                    println!("🌐 Genesis node bound to port: {}", current_port);
                ...
                Err(e) => {
                    if current_port < max_port {
                        ...
                        println!("❌ Critical Error: No available ports found in range 7000-7003 (genesis nodes only).");
                        return Err(e.into());
                    }
                }
            }
        ...
        // Public mainnet node/miner: use AXIOM_PORT or OS-assigned port
        ...
        match swarm.listen_on(addr.clone()) {
            Ok(_) => {
                println!("🌐 Public node bound to port: {}", port);
                println!("🆔 PeerId: {}", swarm.local_peer_id());
                println!("🔊 Listening on: {}", addr);
                for laddr in libp2p::Swarm::listeners(&swarm) {
                    println!("🔊 Swarm listening: {}", laddr);
